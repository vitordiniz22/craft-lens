...
class LensTwigExtensionTest extends Unit
{
    ...
    // -- status.* keys (used in `header.twig` switches) --

    public function testAllStatusKeysResolveInTwig(): void
    {
        $template = <<<'TWIG'
{{ lens.status.pending }}|{{ lens.status.processing }}|{{ lens.status.completed }}|{{ lens.status.failed }}
TWIG;
    ...
    public function testStatusLabelRendersInsideMapFilter(): void
    {
        $output = Craft::$app->view->renderString(
            "{{ ['completed', 'failed']|map(s => lens.statusLabel(s))|join(',') }}"
        );

        $this->assertSame(
            sprintf('%s,%s', AnalysisStatus::Completed->label(), AnalysisStatus::Failed->label()),
            $output,
        );
    }
    ...
    // -- isAutoGeneratedTitle(asset) — argument passing via Twig --
