...
class AssetAnalysisService extends Component
{
    ...
    private function processImageAsset(Asset $asset, AssetAnalysisRecord $record): void
    {
        ...
        try {
            ...
            // Sync tags (from AI) and colors (locally extracted) to indexed tables (respects user-added items)
            $this->syncTagsFromAiResult($record, $result->tags);
            $this->syncExtractedColors($record, $localColors);
    ...
    public function autoApplyFromRecord(Asset $asset, AssetAnalysisRecord $record, array $applyOverrides = []): void
    {
        ...
        $this->autoApplyToSecondarySites(
            assetId: $asset->id,
            primaryTitle: $record->suggestedTitle ?? '',
            primaryAlt: $record->altText ?? '',
            analysisId: $record->id,
            forceTitle: $applyOverrides['applyTitle'] ?? false,
            forceAlt: $applyOverrides['applyAlt'] ?? false,
        );
    }
    ...
    /**
     * Sync locally-extracted colors to the indexed colors table.
     *
     * Deletes existing auto-generated colors (isAutoGenerated=true) and inserts new ones.
     * User-added colors (isAutoGenerated=false) are preserved.
     *
     * @param array<array{hex: string, percentage: float}> $colors
     */
    public function syncExtractedColors(AssetAnalysisRecord $record, array $colors): void
    {
        // Delete only auto-extracted colors; preserve user-added ones
        AssetColorRecord::deleteAll(['analysisId' => $record->id, 'isAutoGenerated' => true]);
        ...
        foreach ($colors as $colorData) {
            ...
            $colorRecord->isAutoGenerated = true;

            if (!$colorRecord->save(false)) {
                Logger::warning(LogCategory::AssetProcessing, 'Failed to save auto-extracted color record', assetId: $record->assetId, context: [
                    'hex' => $hex,
                ]);
            }
        }
